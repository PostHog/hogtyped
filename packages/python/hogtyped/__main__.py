...
def main():
    parser = argparse.ArgumentParser(
        prog="hogtyped", description="Generate a type-safe PostHog wrapper with embedded schemas"
    )
    ...
    generate_parser = subparsers.add_parser(
        "generate", help="Generate a Python wrapper from JSON schemas"
    )
    ...
    generate_parser.add_argument(
        "-c", "--class-name", default="PostHog", help="Generated class name (default: PostHog)"
    )
    generate_parser.add_argument(
        "-m",
        "--mode",
    ...
    if args.command == "generate":
        ...
        example_schema = {
            ...
            "events": {"page_viewed": {"$ref": "#/definitions/PageView"}},
        }
