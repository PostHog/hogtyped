...
describe("TypeScript Code Generator", () => {
  const testSchemasPath = path.join(
    __dirname,
  ...
  describe("Basic Generation", () => {
    test("should generate TypeScript wrapper with embedded schemas", async () => {
      ...
      expect(generatedCode).toContain(
        "export interface ComplexEventProperties",
      );
  ...
  describe("Type Generation", () => {
    test("should generate correct TypeScript types from JSON schema", async () => {
      ...
      expect(generatedCode).toMatch(
        /export interface SimpleEventProperties \{[\s\S]*?name: string;/,
      );
      expect(generatedCode).toMatch(
        /export interface SimpleEventProperties \{[\s\S]*?count: number;/,
      );
      expect(generatedCode).toMatch(
        /export interface SimpleEventProperties \{[\s\S]*?isActive\?: boolean;/,
      );

      // Check ComplexEvent interface
      expect(generatedCode).toMatch(
        /export interface ComplexEventProperties \{[\s\S]*?id: string;/,
      );
      expect(generatedCode).toMatch(
        /export interface ComplexEventProperties \{[\s\S]*?status: 'pending' \| 'active' \| 'completed' \| 'cancelled';/,
      );
      expect(generatedCode).toMatch(
        /export interface ComplexEventProperties \{[\s\S]*?metadata\?: Record<string, any>;/,
      );
      expect(generatedCode).toMatch(
        /export interface ComplexEventProperties \{[\s\S]*?tags\?: string\[\];/,
      );
    });
    ...
    test("should generate EventMap with all events", async () => {
      ...
      expect(generatedCode).toContain(
        "'complex_event': ComplexEventProperties;",
      );
      expect(generatedCode).toContain(
  ...
  describe("Class Generation", () => {
    test("should generate class with custom name", async () => {
      ...
      expect(generatedCode).toContain(
        "export const hogtyped = new CustomAnalytics()",
      );
    ...
    test("should generate capture method with overloads", async () => {
      ...
      expect(generatedCode).toContain(
        "K extends EventName ? EventMap[K] : any",
      );
    });
