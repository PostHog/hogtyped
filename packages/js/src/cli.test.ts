...
describe("CLI Tool", () => {
  ...
  test("should run init command successfully", () => {
    ...
    expect(
      fs.existsSync(path.join(tempDir, "schemas/events.schema.json")),
    ).toBe(true);

    // Verify schema content
    const schema = JSON.parse(
      fs.readFileSync(
        path.join(tempDir, "schemas/events.schema.json"),
        "utf-8",
      ),
    );
  ...
  test("should run generate command with default options", () => {
    ...
    expect(fs.existsSync(path.join(tempDir, "src/posthog.generated.ts"))).toBe(
      true,
    );
  ...
  test("should handle errors gracefully", () => {
    ...
    execSync(`node ${cliPath} generate --schemas "./nonexistent/*.json"`, {
      cwd: tempDir,
      encoding: "utf-8",
    });
